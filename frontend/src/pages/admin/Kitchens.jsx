...
const Kitchens = () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id) => {
      const res = await api.delete(`/admin/kitchen/${id}`);
      return res.data;
    },
    onSuccess: () => {
    ...
    onError: (error) => {
      toast.error(error.response?.data?.detail || error.response?.data?.error || 'Failed to delete kitchen');
    }
  });

  const handleDelete = (id) => {
    if (window.confirm("Are you sure you want to delete this kitchen? This may affect menu items and employees assigned to it.")) {
