...
export default function WaiterProfile() {
 ...
 const avatarUrl = user?.image_url ? `${import.meta.env.VITE_API_URL.replace('/api/v1', '')}${user.image_url}` : null;
 ...
 const handleImageUpload = async (e) => {
     ...
     try {
         const res = await waiterApi.uploadAvatar(formData);
         updateUser({ image_url: res.image_url });
     } catch (err) {
 ...
 <div className="h-28 w-28 rounded-full bg-rose-100/50 flex items-center justify-center text-4xl font-black text-rose-500 border-4 border-white/60 shadow-lg backdrop-blur-md overflow-hidden">
 {avatarUrl ? (
     <img src={avatarUrl} alt="Profile" className="h-full w-full object-cover" />
 ) : initials}
 </div>
 <label className="absolute bottom-0 right-0 h-10 w-10 bg-gradient-to-br from-rose-400 to-rose-500 text-white rounded-full flex items-center justify-center shadow-md border-2 border-white/80 transition-transform cursor-pointer hover:scale-105 active:scale-95">
 <input type="file" className="hidden" accept="image/*" onChange={handleImageUpload} disabled={uploading} />
 <Camera className={`h-5 w-5 ${uploading ? 'animate-pulse' : ''}`}/>
 </label>
 </div>
 ...
 <button key={index} onClick={() => handleMenuClick(item.label)} className="w-full flex items-center justify-between p-4 rounded-3xl transition-all active:scale-[0.98]">
